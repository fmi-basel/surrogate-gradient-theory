...
def set_up_model(cfg: DictConfig, device, dtype):
    #########################################################
...
def monitor_model(cfg: DictConfig, model, data, path, same=True):
    res = model.monitor(data)

    if same:
        print("#" * 100, "Firing rates", "#" * 100)
        firing_rates = []
        mean_over_trials = []
        var_over_trials = []
        fano = []
            ...
            for ax in axs:
                stork.plotting.plot_activity_over_trials(
                    ...
                    neuron_idx=np.random.randint(
                        0, model.groups[cfg.plotting.idx_layer].nb_units
                    ),
                    nolabel=True,
                ),
...
def train(
    cfg: DictConfig, model, train_dataset, valid_dataset, results, epoch_chunk=100000
):
...
def evaluate_training(cfg: DictConfig, model, test_data, results, other=False):
...
def main(cfg):
    ...
    # Set up seeds
    log.info("set up seeds: " + str(cfg.seed))
    ...
    if cfg.dataset.name == "shd":
        ...
        train_data, valid_data, test_data = prepare_data_randman(cfg)
    ...
    model, act_fn = set_up_model(cfg, device, dtype)

    results = {}

    epoch_chunk = 10
    if cfg.anneal:
        epoch_chunk = cfg.anneal_interval
    ...
    for i in range(0, cfg.nb_epochs, epoch_chunk):
        ...
        monitor_model(cfg, model, test_data, f"Plots/{i}", same=False)
        print(cfg.dataset.name)
        if cfg.dataset.name == "shd":  # and cfg.method.name != "superspike":
            monitor_model(cfg, model, same_data, f"Plots/{i}_same", same=True)
        ...
        model, results = train(
            cfg, model, train_data, valid_data, results, epoch_chunk=epoch_chunk
        )
        ...
        if cfg.anneal:
            print("-" * 50 + "Annealing" + "-" * 50)
            act_fn_new = anneal(act_fn, cfg)
            print(act_fn_new.surrogate_params)
            print(act_fn_new.escape_noise_params)
    ...
    monitor_model(cfg, model, test_data, "Plots/after", same=False)
    evaluate_training(cfg, model, test_data, results)
    log.info("Evaluated results")

    if cfg.dataset.name == "shd" and cfg.method.name != "superspike":
        monitor_model(cfg, model, same_data, "Plots/after_same", same=True)
    ...
    monitor_model(cfg, model, test_data, "Plots/after_otherafn", same=False)
    results = evaluate_training(cfg, model, test_data, results, other=True)
    log.info("Evaluated results (other_activation)")

    if cfg.dataset.name == "shd":
        monitor_model(cfg, model, same_data, "Plots/same_after_otherafn", same=True)
